...
num_pts=10000; dt=0.02; N_sample=4;
GV=['.090','.000','.ver']

for ishot in range(1,nShot+1):
    
    dir_ob='Vel_ob/Vel_ob_'+str(ishot)
    mkdir_p(dir_ob)
    ...
    mainfolder_o_new='Vel_ob/Vel_ob_'+str(ishot)+'/'

    os.system('rm Vel_ob/Vel_ob_'+str(ishot)+'/*')
...
np.savetxt('R_all_148s.txt', R_arr)
