...
num_pts=2000; dt=0.08; N_sample=16;
t = np.arange(num_pts)*dt
#R_Time_record[0,:,:]=0
#R_Time_record[1,:,:]=np.max(t)
...
for ishot in range(1,nShot+1):
    ...
    dir_ob='Vel_ob_new_V2/Vel_ob_'+str(ishot)
    mkdir_p(dir_ob)
    ...
    mainfolder_o_new='Vel_ob_new_V2/Vel_ob_'+str(ishot)+'/'

#    os.system('rm Vel_ob_new/Vel_ob_'+str(ishot)+'/*')
...
#            n_shift = -int(shift/dt)
            ...
            if (n_shift<num_pts and n_shift>-int(num_pts0/N_sample)):
                if n_shift>0: 
                    R_Time_record[0,ishot-1,i]=t[n_shift]
                    if (int(num_pts0/N_sample)<num_pts-n_shift):
                        ...
                        R_Time_record[1,ishot-1,i]=t[n_shift+int(num_pts0/N_sample)]
...
#                        R_Time_record[0,ishot-1,i]=t[n_shift]
                        R_Time_record[1,ishot-1,i]=t[num_pts-100]
                                                   
                else: 
                    R_Time_record[0,ishot-1,i]=t[0]
                    if (int(num_pts0/N_sample)<num_pts-n_shift):
                        ...
                        stat_data_0_shift[0:num_pts-100] = stat_data_0_O[-n_shift:num_pts-100-n_shift]
                        stat_data_1_shift[0:num_pts-100] = stat_data_1_O[-n_shift:num_pts-100-n_shift]
                        stat_data_2_shift[0:num_pts-100] = stat_data_2_O[-n_shift:num_pts-100-n_shift]   
...
#                        R_Time_record[0,ishot-1,i]=t[0]
                ...
                #Assign to 0 all over-shifted signals
                stat_data_0_shift = np.zeros(num_pts)
                stat_data_0_shift = np.zeros(num_pts)
                stat_data_0_shift = np.zeros(num_pts)        

#            plt.plot(stat_data_0_shift)
#            plt.show()
            print(R_Time_record[:,ishot-1,i])
...
#np.savetxt('R_all_148s_dh_2km.txt', R_arr)

#np.savetxt('R_Time_record_148s_dh_2km_num_pts_V2.txt', R_Time_record.reshape(-1))            
