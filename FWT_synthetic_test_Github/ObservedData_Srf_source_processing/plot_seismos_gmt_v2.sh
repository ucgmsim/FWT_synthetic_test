...
gmt gmtset PS_MEDIA $paper PROJ_LENGTH_UNIT inch FONT_TITLE 14p FONT_LABEL 16p
nhead=4
red=255/0/0
black=0/0/0
blue=0/0/255
...
gmt psbasemap -R$region -J$proj -B${t_step}::.${label}:"Time (s)":/S -K $orient $origin > $out
tail -$nwin $win | awk '{printf "%f %f\n%f %f\n%f %f\n%f %f\n>\n", $2,-1*m,$2,m,$3,m,$3,-1*m}' m=$max | gmt psxy -R$region -J$proj -W1 -G$paleblue -O -K >> $out
#gmt psxy $obs -R$region -J$proj -H$nhead -W2 -O -K >> $out
#gmt psxy $syn -R$region -J$proj -H$nhead -W2/$red -O -K >> $out
gmt psxy $obs -R$region -J$proj -h$nhead -W2 -O -K >> $out
gmt psxy $syn -R$region -J$proj -h$nhead -W2,$red -O -K >> $out
...
tail -$nwin $win | awk '{printf "> %f %f 8 0 0 LB 10p 0.5i l\n%s\n", $2,-m,substr($0,31,200)}' m=$max | gmt pstext -R$region -J$proj -m -N -O -K -V >> $out
tail -$nwin $qual | awk '{printf "> %f %f 8 90 0 RT 10p 0.5i l\nCC=%.2f\ndT=%.2f\ndA=%.2f\n",$2,m,$5,$4,$6}' m=$max | gmt pstext -R$region -J$proj -F+f14p+jRT -N -O -K >> $out
#tail -$nwin $qual | awk '{printf "> %f %f 8 90 0 RT 10p 0.5i l\nCC=%.2f\n", $2,m,$5,$4}' m=$max | pstext -R$region -J$proj -m -N -O -K >> $out
gmt pstext -R$region -J$proj -F+f14p+jLT -N -O -K <<  END >> $out
$t_start $max Seismograms
END
...
#awk '{print $1, $2}' $stalta | gmt psxy -R$region -J$proj -h$nhead -W2/$blue -O -K >> $out
awk '{print $1, $2}' $stalta | gmt psxy -R$region -J$proj -h$nhead -W2,$blue -O -K >> $out
awk '{print $1, $3}' $stalta | gmt psxy -R$region -J$proj -h$nhead -W2ta,$blue -O -K >> $out

region=$t_start/$t_end/0/1
gmt pstext -R$region -J$proj -N -O -K <<  END >> $out
...
gmt psbasemap -R$region -J$proj -B${t_step}:"Time (s)":/S -O -K $shift >> $out
tail -$nwin $win | awk '{printf "%f %f\n%f %f\n%f %f\n%f %f\n>\n", $2,-1*m,$2,m,$3,m,$3,-1*m}' m=$max | gmt psxy -R$region -J$proj -W1 -G$paleblue -O -K >> $out
gmt psxy $envobs -R$region -J$proj -h$nhead -W2 -O -K >> $out
gmt psxy $envsyn -R$region -J$proj -h$nhead -W2,$red -O -K >> $out
gmt pstext -R$region -J$proj -F+f14p+jLT -N -O -K <<  END >> $out
$t_start $max Envelopes
#$t_start $max 12 0 0 LT Envelopes
END
gmt psbasemap -R$region -J$proj -B${t_step}:"Time (s)":/S -O -U/0/-0.75/$basename >> $out
