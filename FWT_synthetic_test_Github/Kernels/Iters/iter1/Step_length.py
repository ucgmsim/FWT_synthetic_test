...
def Step_length(Vs,Vp,rho,dx,dy,dz,nt,Gra_Vp, Gra_Vs,Err,S,nShot):
...
#Confirm optimal step length
    ...
    print('max st_Vp1*Gra_Vp='+str(np.max(np.abs(st_Vsp2*grad_Vsp))))  
    ...
    #time.sleep(70)
    f_err=open('err_opt.dat','r')
    Err2=np.fromfile(f_err,dtype='<f4')
    if (Err2[1]>Err):
