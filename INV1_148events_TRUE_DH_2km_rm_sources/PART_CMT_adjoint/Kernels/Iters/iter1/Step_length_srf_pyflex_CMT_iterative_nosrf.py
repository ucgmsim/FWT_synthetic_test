...
def write_par_full_CMT_source_i(Si,M_full):
    ...
    fid.write("%s\n" %('Mnn='+str('{:.2e}'.format(M[0,0]))))
    fid.write("%s\n" %('Mee='+str('{:.2e}'.format(M[1,1]))))
    fid.write("%s\n" %('Mdd='+str('{:.2e}'.format(M[2,2]))))
    fid.write("%s\n" %('Mne='+str('{:.2e}'.format(M[0,1]))))
    fid.write("%s\n" %('Mnd='+str('{:.2e}'.format(M[0,2]))))
    fid.write("%s\n" %('Med='+str('{:.2e}'.format(M[1,2]))))
...
def Step_length_CMT():
    ...
    grad_M_arr1 = np.loadtxt('Dump_source/Grad_M.it'+str(iter_s)+'.txt')
    M0_arr = np.loadtxt('Dump_source/M0.it'+str(iter_s)+'.txt')
    
    gra_M=grad_M_arr1.reshape([3,3])
    M0=M0_arr.reshape([3,3])
    print(M0)
    f_err=open('Dump_source/cmt_err.dat.it'+str(iter_s),'r'); Err0=np.fromfile(f_err,dtype=np.float64); print('Err='+str(Err0));    
    ...
    while (L21>L20 and count<iter_max and flag==0):
        ...
        #input('-->')
        write_par_full_CMT_source_i(S[ishot-1,:],M0_new)
...
##############################            
    ...
    np.savetxt('Dump_source/M0.it'+str(iter_s+1)+'.txt',M0_new.reshape(-1))
    fi=open('Dump_source/step_cmt_it'+str(iter_s)+'.dat','w'); (np.float64(st)).tofile(fi); fi.close()
