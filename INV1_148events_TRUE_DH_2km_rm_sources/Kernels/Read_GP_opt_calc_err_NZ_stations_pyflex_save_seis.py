...
irec_arr = [4, 5, 12, 13, 15, 20, 28, 37, 42, 53, 58, 65, 66, 47, 61, 14, 16, 22, 57, 59, 8, 39, 60]

Nr = len(statnames_RGT)
Ns = len(ishot_arr)
DD = np.zeros([Ns,Nr,3,num_pts])
DD_TauP = np.zeros([Ns,Nr,3])
DD_rwm = np.zeros([Ns,Nr,3])
...
#    if ((S[ishot-1,2]<20) and (np.sum(R_all[:,:,ishot-1])>6)):
        ...
        for ir,statname in enumerate(statnames_RGT):
...
#            distance=((R[i,1]-S[ishot-1,1])**2+(R[i,2]-S[ishot-1,2])**2+(R[i,0]-S[ishot-1,0])**2)**(0.5)      
            ...
            for k in range(0,3): 
                s0=statname+GV[k]           
                #if(2>1):                  
                if ((R_all[irec_arr[ir],k,ishot-1]==1)): 
                    print(statname+':irec='+str(irec_arr[ir]))
                    ...
                    DD[ishot_id,ir,k,:] = stat_data_0_S
            ...
            #        stat_data_0_O = rms(stat_data_0_O)
                    ...
                    DD_TauP[ishot_id,ir,k] = td_shift
                    DD_rwm[ishot_id,ir,k] = rwm_wd(stat_data_0_S,stat_data_0_O,num_pts, dt,wd)
...
np.savetxt('DD_iter.txt', DD.reshape(-1))
np.savetxt('DD_TauP_iter.txt', DD_TauP.reshape(-1))
np.savetxt('DD_rwm_iter.txt', DD_rwm.reshape(-1))    
