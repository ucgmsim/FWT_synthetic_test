...
statnames_RGT = ['CSTC', 'KPOC', 'SPRS', 'REHS', 'WCSS', 'DSLC', 'CACS', 'CBGS', 'TPLC', 'NNBS', 'DFHS', 'EYRS', 'CRLZ', 'MTHS', 'HPSC', 'KOWC', 'SHFC', 'CSHS', 'ASHS', 'ADCS', 'LINC', 'SWNC', 'SLRC']
irec_arr = []
Nr = len(statnames_RGT)
Ns = len(ishot_arr)

DD_all = np.zeros([iters,Ns*Nr*3*nts])
DD_TauP_all = np.zeros([iters,Ns*Nr*3])
DD_rwm_all = np.zeros([iters,Ns*Nr*3])
...
for it in range(1,iters+1):
    ...
    if (it==1):
        os.system('cp ../../../Model/Models/vs3dfile_h.s Model/vs3dfile_opt.s')
        os.system('cp ../../../Model/Models/vp3dfile_h.p Model/vp3dfile_opt.p')
        os.system('cp ../../../Model/Models/rho3dfile_h.d Model/rho3dfile_opt.d')
...
#    input('Copy est. data, calculate error function and Press 1 to continue')
    ...
    job_file = 'optimized_misfit_save_seis.sl'
    job_submittted(job_file)
    DD = np.loadtxt('DD_iter.txt')
    DD_TauP = np.loadtxt('DD_TauP_iter.txt')
    DD_rwm = np.loadtxt('DD_rwm_iter.txt')
    
    DD_all[it-1,:] = DD
    DD_TauP_all[it-1,:] = DD_TauP
    DD_rwm_all[it-1,:] = DD_rwm
    input('next iter')

#np.savetxt('DD_all.txt', DD_all.reshape(-1))
#np.savetxt('DD_TauP_all.txt', DD_TauP_all.reshape(-1))
#np.savetxt('DD_rwm_all.txt', DD_rwm_all.reshape(-1))
np.savetxt('DD_all_23.txt', DD_all.reshape(-1))
np.savetxt('DD_TauP_all_23.txt', DD_TauP_all.reshape(-1))
np.savetxt('DD_rwm_all_23.txt', DD_rwm_all.reshape(-1))
