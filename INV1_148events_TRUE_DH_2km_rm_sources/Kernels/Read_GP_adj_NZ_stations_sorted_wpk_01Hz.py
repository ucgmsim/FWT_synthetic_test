...
#        stat_data_2_O = signal.filtfilt(b, a, stat_data_2_O)            
        ...
        window_pad0=np.exp(-(0.5*sd**2)*(t-t[I_max])**2)
        #Isolation filters  
        IsolationFilter_0 = np.multiply(stat_data_0_S,window_pad0)    

        I_max=np.argmax(stat_data_1_O)
        window_pad1=np.exp(-(0.5*sd**2)*(t-t[I_max])**2)
        #Isolation filters  
        IsolationFilter_1 = np.multiply(stat_data_1_S,window_pad1)

        I_max=np.argmax(stat_data_2_O)
        window_pad2=np.exp(-(0.5*sd**2)*(t-t[I_max])**2)
        #Isolation filters  
        IsolationFilter_2 = np.multiply(stat_data_2_S,window_pad2)
        
        #if(rwm_wd(stat_data_0_Sf,stat_data_0_Of,num_pts, dt,window_pad0)==0):
        #    source_x=np.zeros(num_pts)
        #else:
        #    source_x,_ = source_adj_gsdf(stat_data_0_S,stat_data_0_O,IsolationFilter_0,num_pts,dt)

        #if(rwm_wd(stat_data_1_Sf,stat_data_1_Of,num_pts, dt,window_pad1)==0):
        #    source_y=np.zeros(num_pts)
        #else:
        #    source_y,_ = source_adj_gsdf(stat_data_1_S,stat_data_1_O,IsolationFilter_1,num_pts,dt)

        #if(rwm_wd(stat_data_2_Sf,stat_data_2_Of,num_pts, dt,window_pad2)==0):
