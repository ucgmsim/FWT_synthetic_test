...
nShot_opt=nShot
#nShot_opt=37
rwm_all=np.zeros(nShot_opt*nRec*3)
count=0
fid = open('ncc_file.txt','w')
for ishot in range(1,nShot+1):
#for ishot in range(75,111+1):
    ...
    #            stat_data_2_O = signal.filtfilt(b, a, stat_data_2_O)       
                ...
                rwm_all[count] = rwm(stat_data_0_S,stat_data_0_O,num_pts, dt)+rwm(stat_data_1_S,stat_data_1_O,num_pts, dt)+rwm(stat_data_2_S,stat_data_2_O,num_pts, dt)        
         
                Err = Err+ rwm_all[count]
                fid.write("%4d%20s%20s \n" %(count,'source'+str(ishot),'station '+statname))
                count = count+1
     
#f_err = open('err_opt.dat','w')
#Err.astype('float').tofile(f_err)     
#print(Err)    
np.savetxt('rwm_all.txt', rwm_all)    
