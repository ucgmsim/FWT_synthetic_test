...
#    R_ishot_arr=np.loadtxt('../../../../Kernels/Iters/iter1/Dump/R_ishot_'+str(ishot)+'.txt')
    ...
    #os.system('cp ../../../../Kernels/Vel_ob_ref/Vel_ob_'+str(ishot)+'/*.* ../../Vel_ob_new_V2/Vel_ob_i/')
    ...
    for iter_s in range(0,4):
        ...
        if(iter_s==0):
            input('Use data as observed data if not modifying the srf ')
...
#        input('adjoint simulation-->')
        ...
        os.system('python ../../Read_GP_adj_NZ_stations_sorted_pyflex_cmt_synthetic.py')
        time.sleep(5)
    ...
    #input('Press 1 then Enter to continue')
        job_file2 = 'kernel_shot_i_CMT_nsource.sl'
        submit_time = datetime.now().strftime('%Y-%m-%d')
