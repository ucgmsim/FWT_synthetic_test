...
R_all_arr=np.loadtxt('../../index_all_ncc_pyflex_gt05.txt')
#R_all_arr=np.loadtxt('../../index_all_ncc_pyflex_gaussian_Tshift.txt')
R_all=R_all_arr.reshape([nRec,3,nShot])

#for ishot in range(1,nShot+1):
#for ishot in range(95,104+1):
ishot_arr=[28, 120, 132, 139, 143]
for ishot_id in range(0,5):
    ...
    if ((S[ishot-1,2]<20) and (np.sum(R_all[:,:,ishot-1])>12)):
        ...
        for i,statname in enumerate(statnames):
            ...
            #        stat_data_0_O = signal.filtfilt(b, a, stat_data_0_O)
                    ...
                    filename = '../../ALL_WINs_pyflex/'+e_s_c_name
