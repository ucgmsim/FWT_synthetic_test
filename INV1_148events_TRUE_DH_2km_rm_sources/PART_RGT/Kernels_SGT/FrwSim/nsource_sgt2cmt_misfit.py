...
for i,statname in enumerate(statnames_RGT):
    for k in range(0,3):
        ...
        for ishot_id in range(0,len(ishot_arr)):
            ...
            AA[ishot_id,i,k,:,3] = time_shift_emod3d(Mxy[i_source,:],delay_Time,dt)
            
            AA[ishot_id,i,k,:,2] = time_shift_emod3d(Mxz[i_source,:],delay_Time,dt)
            AA[ishot_id,i,k,:,6] = time_shift_emod3d(Mxz[i_source,:],delay_Time,dt)
            
            AA[ishot_id,i,k,:,5] = time_shift_emod3d(Myz[i_source,:],delay_Time,dt)
            AA[ishot_id,i,k,:,7] = time_shift_emod3d(Myz[i_source,:],delay_Time,dt)           
...
np.savetxt('CMT_inv_1events_nstats.txt',M_all_array)
    
print('Err='+str(Err))
f_err = open('err_srf.dat','w')            
