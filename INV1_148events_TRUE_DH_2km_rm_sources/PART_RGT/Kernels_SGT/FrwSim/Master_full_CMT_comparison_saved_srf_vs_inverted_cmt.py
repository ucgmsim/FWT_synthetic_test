...
with open('statnames_RGT.txt', 'w') as f:
    for stat in statnames_RGT:
        f.write(stat + '\n')
#input('-->')
...
for ishot_id in range(0,len(ishot_arr)):
    ...
    print(S[ishot-1,:])
    mkdir_p('../Vel_es/Vel_es_'+str(ishot))
    mkdir_p('../Vel_opt/Vel_opt_'+str(ishot))    
...
#    job_submittted(job_file)    
    os.system('python source_i_sgt2cmt_misfit.py')
    print("srf misfit and new cmt solution") 

    M_array = np.loadtxt('CMT_inv_1events_nstats.txt')
    M_full_new = M_array.reshape([3,3])    
    
M_full_new = M_full_new * LA.norm(M_full)/LA.norm(M_full_new)
    #M_full_new = M_full_new * Mo/LA.norm(M_full_new)

    write_par_full_CMT_source_i(S[ishot-1,:],M_full_new)
    
    job_file11 = 'FWT_emod3d_shot_i_full_CMT_part1_stf.sl'
    job_submittted(job_file11)
    os.system('mv ../Vel_es/Vel_es_i/*.*  ../Vel_opt/Vel_opt_'+str(ishot))

#    job_file = 'adj_cmt.sl'
#    job_submittted(job_file)
    os.system('python source_i_cmt_misfit.py')
    print("cmt misfit") 

#f_err=open('err_srf.dat','r'); Err1=np.fromfile(f_err,dtype=np.float64); print('Err1='+str(Err1))    
#f_err=open('err_cmt.dat','r'); Err2=np.fromfile(f_err,dtype=np.float64); print('Err2='+str(Err2))    
