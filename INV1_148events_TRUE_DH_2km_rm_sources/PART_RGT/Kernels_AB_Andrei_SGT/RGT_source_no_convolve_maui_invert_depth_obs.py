...
#            matrix_file_fw='/home/user/workspace/GMPlots/synth_VMs/Medium_VMs/fwd01_xyzts.exx'
            ...
            for ishot_id in range(0,len(ishot_arr)):
                ...
                for depth_id in range(-2,3):
        
                    eii3_fw=eii3[:,int(S[ishot-1,1])-1, depth_id+int(S[ishot-1,2])-1,int(S[ishot-1,0])-1]
                    
    #                AA[ishot_id,i,k,:,int(ij_array[ij])]=conv_source(eii3_fw,source,nt0,dt0)
                    AA[ishot_id*5+int(depth_id)+2,i,k,:,int(ij_array[ij])]=eii3_fw
                    
                    if(int(ij_array[ij])==1):
    #                    AA[ishot_id,i,k,:,3]=conv_source(eii3_fw,source,nt0,dt0)
                        AA[ishot_id*5+int(depth_id)+2,i,k,:,3]=eii3_fw
                    if(int(ij_array[ij])==2):
                        AA[ishot_id*5+int(depth_id)+2,i,k,:,6]=eii3_fw     
                    if(int(ij_array[ij])==5):
                        AA[ishot_id*5+int(depth_id)+2,i,k,:,7]=eii3_fw                      
                    
np.savetxt('AA_1events.txt',AA.reshape(-1))
input('-->')
##############################################
##############################################
...
#    os.system('cp Vel_ob_ref_01Hz/Vel_ob_'+str(ishot)+'/*.* /home/user/workspace/GMPlots/Sim/Vel_es/') 
    os.system('cp Vel_ob_ref_01Hz_RGT/Vel_ob_'+str(ishot)+'/*.* /home/user/workspace/GMPlots/Sim/Vel_es/')    
...
#    mainfolder_o='../../Kernels/Vel_ob_ref/Vel_ob_'+str(ishot)+'/'
    ...
    index_obs=np.zeros(int(3*Nr*nt0))
...
#    for i,statname in enumerate(statnames):
    for i,statname in enumerate(statnames_RGT):
        ...
        if(statname in statnames_RGT):
            ...
            for k in range(0,3):
                s0=statname+GV[k]
                if (R_all[irec_arr[ik],k,ishot-1]==1):
            ...
            #        stat_data_0_O = signal.filtfilt(b, a, stat_data_0_O)
                    ...
                    wd[int(R_Time_record[0,ishot-1,irec_arr[ik]]/dt):int(R_Time_record[1,ishot-1,irec_arr[ik]]/dt)+1]=1     
...
#                    obs[nt0*k*ik+nt0*k:nt0*k*ik+nt0*(k+1)]=-resample(stat_data_0_S,int(nt0))
                    obs[nt0*k*ik+nt0*k:nt0*k*ik+nt0*(k+1)]=-resample(stat_data_0_O,int(nt0))
                    
                    index_obs[nt0*k*ik+nt0*k:nt0*k*ik+nt0*(k+1)]=1
                
    obs_ext = np.r_[obs,[0,0,0,0]]
    
    misfit = 0
        
    for depth_id in range(-2,3):      

        AA_ishot = AA[ishot_id*5+int(depth_id)+2,:,:,:,:]
    ...
    #    d=np.r_[d, 0]; d=np.r_[d, 0]; d=np.r_[d, 0];
        ...
        AA_ishot_ext = np.r_[AA_ishot_ext, b]     
        #Inversion
        AA_inv=inv(np.matmul(np.transpose(AA_ishot_ext),AA_ishot_ext))     
        AA_d=np.matmul(np.transpose(AA_ishot_ext),obs_ext)
        M=np.matmul(AA_inv,AA_d)
        
        syn = np.matmul(AA_ishot,M) 
        syn_arr = syn.reshape(-1)
        misfit_new = np.sum(np.multiply(np.square((syn_arr-obs)),index_obs))
    ...
    idepth_all[ishot_id] = i_depth
#    print(i_depth)
#    print(M_ishot)
    
M_all_array=M_all.reshape(-1)
np.savetxt('CMT_inv_1events_noconv_invert_depth_obs.txt',M_all_array)
