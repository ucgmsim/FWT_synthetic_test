...
#            matrix_file_fw='/home/user/workspace/GMPlots/synth_VMs/Medium_VMs/fwd01_xyzts.exx'
            ...
            for ishot_id in range(0,len(ishot_arr)):
                ...
                for depth_id in range(-2,3):
                    ...
                    #Shift the strain time series
                    TauPP = 60
                    eii3_fw = time_shift_emod3d(eii3_fw,TauPP,dt0)                    
...
#np.savetxt('AA_1events_Tshift.txt',AA.reshape(-1))
np.savetxt('AA_1events_Tshift_3stat.txt',AA.reshape(-1))
...
#                obs[nt0*k*ik+nt0*k:nt0*k*ik+nt0*(k+1)]=resample(stat_data_0_S,int(nt0)) 
    ...
    #    d=np.r_[d, 0]; d=np.r_[d, 0]; d=np.r_[d, 0];
        ...
        misfit_new = np.square((syn_arr-obs))
...
np.savetxt('CMT_inv_1events_noconv_invert_depth.txt',M_all_array)
