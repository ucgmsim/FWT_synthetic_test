...
#            matrix_file_fw='/home/user/workspace/GMPlots/synth_VMs/Medium_VMs/fwd01_xyzts.exx'
            ...
            for ishot_id in range(0,len(ishot_arr)):
                ...
                #Convert from strain tensor to strain Green tensor
                rho_s=rho[int(S[ishot-1,1])-1,int(S[ishot-1,2])-1,int(S[ishot-1,0])-1]
                Vs_s=Vs[int(S[ishot-1,1])-1,int(S[ishot-1,2])-1,int(S[ishot-1,0])-1]
                
                if(eij_name=='exx' or eij_name=='eyy' or eij_name=='ezz'):
                    eii3_fw=eii3_fw*(-4*rho_s*Vs_s)
                if(eij_name=='exy' or eij_name=='exz' or eij_name=='eyz'):
                    eii3_fw=eii3_fw*(2*rho_s*Vs_s)
#                if(eij_name=='exx'):
#                    eii3_fw=eii3_fw*(4*rho[int(S[ishot-1,1])-1,int(S[ishot-1,2])-1,int(S[ishot-1,0])-1]*Vs[int(S[ishot-1,1])-1,int(S[ishot-1,2])-1,int(S[ishot-1,0])-1])                
                ...
                if(int(ij_array[ij])==2):
                    AA[ishot_id,i,k,:,6]=conv_source(eii3_fw,source,nt0,dt0)     
                if(int(ij_array[ij])==5):
                    AA[ishot_id,i,k,:,7]=conv_source(eii3_fw,source,nt0,dt0)                   
...
#    if ((S[ishot-1,2]<20) and (np.sum(R_all[:,:,ishot-1])>3)):
    ...
    #            d[nt0*k*i+nt0*k:nt0*k*i+nt0*(k+1)]=resample(stat_data_0_O,int(nt0/num_pts))
...
np.savetxt('CMT_inv_8events_new.txt',M_all_array)
